...
mod test_utils;
use test_utils::env::{demo_credentials, demo_account_number};
...
async fn test_demo_login_and_accounts() -> Result<()> {
    ...
    let accounts = with_timeout(
        INTEGRATION_TIMEOUT,
        tasty.accounts()
    ).await.expect("Accounts timeout")
    .expect("Failed to fetch accounts");

    // Should have at least one account in demo
    assert!(!accounts.is_empty(), "Demo environment should have at least one account");

    println!("✓ Successfully fetched {} account(s) from demo environment", accounts.len());
    ...
    assert!(!account_number.0.is_empty(), "Account number should not be empty");
...
async fn test_demo_account_positions() -> Result<()> {
    ...
    let positions = with_timeout(
        INTEGRATION_TIMEOUT,
        account.positions()
    ).await.expect("Positions timeout")
    ...
    for (i, position) in positions.iter().enumerate() {
        ...
        assert!(!position.symbol.0.is_empty(), "Position symbol should not be empty");
...
async fn test_demo_live_orders() -> Result<()> {
    ...
    for (i, order) in orders.iter().enumerate() {
        ...
        assert!(!order.underlying_symbol.0.is_empty(), "Order symbol should not be empty");
...
async fn test_specific_demo_account() -> Result<()> {
    ...
    match specific_account {
        ...
        None => {
            println!("⚠ Specific account {} not found in demo environment", account_num);
...
async fn test_demo_session_token_usage() -> Result<()> {
    ...
    let accounts = tasty.accounts().await?;
    assert!(!accounts.is_empty(), "Should be able to fetch accounts with session token");
