use tastytrade_rs::{TastyTrade, api::oauth2::{OAuth2ClientBuilder, OAuth2Config, OAuth2Token}};

/// OAuth2 integration demos; all require manual setup and are ignored by default.
...
async fn personal_grant_refresh_flow() {
    ...
    let tasty = TastyTrade::oauth2_from_refresh_token(client, &refresh_token, true).await
        .expect("Failed to create OAuth2 client from refresh token");
...
async fn authorization_code_exchange_flow() {
    ...
    let tasty = TastyTrade::oauth2_exchange_code(config, &code, true).await
