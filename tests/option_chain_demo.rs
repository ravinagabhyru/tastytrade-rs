...
async fn test_demo_nested_option_chain_aapl() -> Result<()> {
    ...
    ).await.expect("Option chain timeout")
    .expect("Failed to fetch AAPL option chain");

    // Validate chain structure
    assert_eq!(chain.underlying_symbol.0, "AAPL");
    assert_eq!(chain.root_symbol.0, "AAPL");
    assert!(!chain.expirations.is_empty(), "AAPL should have option expirations");
    assert_eq!(chain.shares_per_contract, 100, "Standard equity options should be 100 shares per contract");

    println!("✓ AAPL option chain has {} expiration(s)", chain.expirations.len());

    // Validate at least one expiration has strikes
    let first_exp = &chain.expirations[0];
    assert!(!first_exp.strikes.is_empty(), "First expiration should have strikes");
    assert!(!first_exp.expiration_date.is_empty(), "Expiration date should not be empty");
    
    println!("✓ First expiration: {} with {} strike(s)", 
        first_exp.expiration_date, first_exp.strikes.len());

    // Validate strike structure
    let first_strike = &first_exp.strikes[0];
    assert!(first_strike.strike_price > rust_decimal::Decimal::ZERO, "Strike price should be positive");
    assert!(!first_strike.call.0.is_empty(), "Call symbol should not be empty");
    assert!(!first_strike.put.0.is_empty(), "Put symbol should not be empty");
    
    // Verify option symbols follow expected format
    assert!(first_strike.call.0.contains("C"), "Call symbol should contain 'C'");
    assert!(first_strike.put.0.contains("P"), "Put symbol should contain 'P'");
    
    println!("✓ First strike: ${} (Call: {}, Put: {})", 
...
async fn test_demo_flat_option_chain_spy() -> Result<()> {
    ...
    for (i, chain) in chains.iter().take(5).enumerate() {  // Check first 5 entries
        assert_eq!(chain.underlying_symbol.0, "SPY");
        assert!(chain.strike_price > rust_decimal::Decimal::ZERO, "Strike price should be positive");
        
        // The extra map should contain additional fields from the API
        println!("  Entry {}: Strike ${}, Extra fields: {}", 
            i + 1, chain.strike_price, chain.extra.len());
        ...
        match chain_result {
            Ok(chain) => {
                ...
                assert!(!chain.expirations.is_empty(), 
                    "Symbol {} should have option expirations", symbol.0);
                
                println!("✓ {} has {} expiration(s)", symbol.0, chain.expirations.len());
            },
            Err(e) => {
...
async fn test_demo_option_chain_strike_precision() -> Result<()> {
    ...
    let first_exp = &chain.expirations[0];
    let strikes: Vec<_> = first_exp.strikes.iter()
    ...
    for strike in &strikes {
        assert!(*strike > rust_decimal::Decimal::ZERO, "Strike should be positive");
        assert!(*strike < rust_decimal::Decimal::from(10000), "Strike should be reasonable");
        ...
        let deserialized: rust_decimal::Decimal = serde_json::from_str(&serialized)
            .expect("Should deserialize");
        assert_eq!(*strike, deserialized, "Precision should be maintained");
    }
    
    println!("✓ Strike prices maintain precision: {:?}", &strikes[..std::cmp::min(5, strikes.len())]);
...
async fn test_demo_option_chain_date_validation() -> Result<()> {
    ...
    for exp in &chain.expirations {
        ...
        assert!(
            ...
            "Expiration date should be in YYYY-MM-DD format: {}", exp.expiration_date
        );
        
        // Days to expiration should be reasonable (not negative, not too far out)
        assert!(exp.days_to_expiration < 1000, "Days to expiration seems too large: {}", exp.days_to_expiration);
        
        // Settlement type should be valid
        assert!(
            exp.settlement_type == "AM" || exp.settlement_type == "PM",
            "Settlement type should be AM or PM: {}", exp.settlement_type
        );
...
async fn test_demo_invalid_symbol_option_chain() -> Result<()> {
    ...
    match result {
        Ok(chain) => {
            ...
            println!("⚠ Invalid symbol returned chain with {} expirations", chain.expirations.len());
        },
        Err(e) => {
...
async fn test_demo_option_symbol_format_validation() -> Result<()> {
    ...
    for exp in &chain.expirations {
        for strike in &exp.strikes {
            ...
            assert!(call.starts_with("AAPL"), "Call symbol should start with AAPL: {}", call);
            assert!(put.starts_with("AAPL"), "Put symbol should start with AAPL: {}", put);
            
            // Call should contain 'C', put should contain 'P'
            assert!(call.contains('C'), "Call symbol should contain 'C': {}", call);
            assert!(put.contains('P'), "Put symbol should contain 'P': {}", put);
            
            // Symbols should be reasonable length (not too short/long)
            assert!(call.len() > 10 && call.len() < 30, "Call symbol length seems wrong: {}", call);
            assert!(put.len() > 10 && put.len() < 30, "Put symbol length seems wrong: {}", put);
