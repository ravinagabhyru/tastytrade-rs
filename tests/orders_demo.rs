...
async fn test_demo_equity_buy_dry_run() -> Result<()> {
    ...
    let dry_run_result = with_timeout(INTEGRATION_TIMEOUT, account.dry_run(&order))
        .await
        .expect("Dry run timeout");
    ...
    let dry_run = match dry_run_result {
        ...
        Err(e) => {
            ...
            println!(
                "⚠ Market order dry run failed (possibly due to market hours or restrictions): {}",
                e
            );
    ...
    println!(
        ...
        dry_run.buying_power_effect.current_buying_power
    );

    // Fee calculation should be present
    println!(
        "  Total fees: {} ({:?})",
        dry_run.fee_calculation.total_fees, dry_run.fee_calculation.total_fees_effect
    );
...
async fn test_demo_equity_limit_order_dry_run() -> Result<()> {
    ...
    println!(
        "  Order type: {:?}, TIF: {:?}",
        dry_run.order.order_type, dry_run.order.time_in_force
    );
...
async fn test_demo_multi_leg_option_dry_run() -> Result<()> {
    ...
    match dry_run_result {
        Ok(dry_run) => {
            ...
            println!(
                "  Multi-leg order executed with {} legs",
                dry_run.order.legs.len()
            );
...
async fn test_demo_buying_power_calculations() -> Result<()> {
    ...
    println!(
        ...
        initial_balance.equity_buying_power
    );
    ...
    for &qty in &quantities {
        ...
        match dry_run_result {
            Ok(dry_run) => {
                println!(
                    "✓ {} shares SPY buying power impact: {}",
            ...
            Err(e) => {
                ...
                println!(
                    "⚠ {} shares SPY dry run failed (possibly due to market hours): {}",
                    qty, e
                );
    ...
    for (name, instrument_type, symbol, quantity) in test_cases {
        ...
        match dry_run_result {
            ...
            Err(e) => {
                // Market orders may fail when markets are closed
                println!(
                    "⚠ {} order dry run failed (possibly due to market hours): {}",
                    name, e
                );
